...
window.AkkedWizard = {
    ...
    return `
      <div class="step-pane animate-fade-in">
        ...
        <div class="dropzone" onclick="document.getElementById('file-upload-input').click()">
          ...
          <div style="font-size: 1.1rem; font-weight: 700; color: var(--brand-primary);">${I18N.t('uploadBoxTitle')}</div>
          ...
          ${AkkedSampleDocs.map(doc => `
            <div class="sample-doc-card ${this.selectedDocId === doc.id ? 'selected' : ''}" onclick="AkkedWizard.selectDoc('${doc.id}')">
              <div class="sample-doc-thumb">
                ...
                <span style="font-size: 2.5rem;">${doc.id === 'national_id' ? '🪪' : (doc.id === 'salary_cert' ? '📑' : '💻')}</span>
              </div>
  ...
  renderStep2() {
    const isAr = I18N.currentLang === 'ar';
    const purposes = [
      { id: 'purpose_age', nameAr: 'التحقق من الأهلية العمرية (فوق 18 عاماً)', nameEn: 'Age Eligibility Verification (18+ Only)', icon: '🔞', docReq: 'national_id', defaultRecipient: 'recipient_store' },
      { id: 'purpose_salary', nameAr: 'التحقق من الحد الأدنى للراتب وملاءمة الإيجار', nameEn: 'Income Threshold & Lease Suitability', icon: '💰', docReq: 'salary_cert', defaultRecipient: 'recipient_car_rental' },
      { id: 'purpose_warranty', nameAr: 'التحقق من سريان الضمان والرقم التسلسلي', nameEn: 'Warranty Validity & Serial Check', icon: '🔧', docReq: 'warranty_receipt', defaultRecipient: 'recipient_service_center' }
    ];
    ...
    return `
      <div class="step-pane animate-fade-in">
        ...
        <p style="font-size: 0.88rem; color: var(--text-muted); margin-bottom: 24px;">
          ${isAr ? 'حدد الغرض المصرح به بدقة ليتمكن محرك الذكاء الاصطناعي من حجب كافة البيانات التي لا تخص هذا الغرض.' : 'Specify the authorized purpose so AI can shield all unneeded personal data.'}
        </p>
        ...
        <div class="selection-list">
          ${AkkedState.entities.map(ent => `
            <div class="selection-card ${this.selectedRecipientId === ent.id ? 'selected' : ''}" onclick="AkkedWizard.selectRecipient('${ent.id}')">
              <div class="selection-info">
                <div class="selection-icon">${ent.icon}</div>
                <div>
                  <div style="font-weight: 700; color: var(--text-main); font-size: 0.95rem;">${isAr ? ent.nameAr : ent.nameEn}</div>
                  <div style="font-size: 0.8rem; color: var(--brand-accent); font-weight: 600;">⭐ ${isAr ? 'تقييم الالتزام:' : 'Compliance:'} ${ent.trustScore}%</div>
                </div>
              </div>
        ...
        <div style="display: flex; justify-content: space-between; margin-top: 32px;">
          ...
          <button class="btn btn-primary btn-lg" onclick="AkkedWizard.nextStep()">
            <span>${isAr ? 'تحليل الوثيقة بالذكاء الاصطناعي' : 'Run AI Minimization Analysis'}</span>
            <span>${isAr ? '←' : '→'}</span>
          </button>
        </div>
      </div>
  ...
  // Step 3: AI Minimization & Sensitive Data Detection
  renderStep3() {
    const isAr = I18N.currentLang === 'ar';
    ...
    return `
      <div class="step-pane animate-fade-in">
        ...
        <div style="background: linear-gradient(135deg, rgba(80, 190, 155, 0.15) 0%, rgba(90, 24, 84, 0.08) 100%); border: 1.5px solid var(--brand-accent); border-radius: var(--radius-lg); padding: 20px; margin-bottom: 24px;">
          <div style="display: flex; align-items: flex-start; gap: 14px;">
            <div style="font-size: 2rem;">🤖</div>
            <div>
              <div style="font-weight: 800; color: var(--brand-primary); font-size: 1.05rem; margin-bottom: 4px;">
                ${isAr ? 'توصية الذكاء الاصطناعي لتقليص البيانات:' : 'AI Minimum Disclosure Recommendation:'}
              </div>
              ...
              <div style="font-size: 0.82rem; color: var(--text-muted); line-height: 1.4;">
                💡 ${isAr ? rec.rationalAr : rec.rationalEn}
              </div>
            </div>
          </div>
        </div>
        ...
        <div style="margin-bottom: 14px; display: flex; align-items: center; justify-content: space-between;">
          <div style="font-weight: 700; font-size: 0.95rem; color: var(--text-main);">
            ${I18N.t('detectedFieldsCount', { count: doc.fields.length })}
          </div>
          <div style="font-size: 0.82rem; color: var(--brand-accent); font-weight: 700;">
            🔒 ${isAr ? 'تم حجب كافة الحقول الزائدة' : 'All redundant fields shielded'}
          </div>
        </div>
            ...
            return `
              <div class="field-item-row ${isAllowed ? 'exposed' : 'shielded'}">
                <div class="field-item-meta">
                  <span style="font-size: 1.2rem;">${isAllowed ? '🔓' : '🔒'}</span>
                  <div>
                    <div style="font-weight: 700; color: var(--text-main); font-size: 0.9rem;">
                      ${isAr ? f.nameAr : f.nameEn}
                    </div>
                    <div style="font-size: 0.78rem; color: var(--text-muted);">
                      ${isAllowed ? (isAr ? 'سيتم كشف هذا الحقل فقط للجهة' : 'This claim only will be shared') : (isAr ? 'محجوب تماماً لمنع الانكشاف' : 'Shielded from recipient')}
                    </div>
                  </div>
                </div>
        ...
        <div class="grid-container grid-cols-2" style="margin-bottom: 24px;">
          ...
          <div class="card" style="padding: 16px; border: 2px solid #EF4444; background: #FFF5F5;">
            <div style="display: flex; align-items: center; justify-content: space-between; margin-bottom: 12px;">
              <span class="badge" style="background: #EF4444; color: #FFF;">⚠️ ${I18N.t('labelOriginal')}</span>
              <span style="font-size: 0.78rem; color: #991B1B; font-weight: 700;">100% ${isAr ? 'انكشاف كامل للبيانات' : 'Full Exposure'}</span>
            </div>
          ...
          <div class="card" style="padding: 16px; border: 2px solid #50BE9B; background: #F0FDF4;">
            <div style="display: flex; align-items: center; justify-content: space-between; margin-bottom: 12px;">
              <span class="badge" style="background: #50BE9B; color: #FFF;">🛡️ ${I18N.t('labelProtected')}</span>
              <span style="font-size: 0.78rem; color: #065F46; font-weight: 700;">🔒 ${isAr ? 'تقليص مشفر للحد الأدنى' : 'Minimized & Watermarked'}</span>
            </div>
            <div style="height: 280px; overflow: hidden; border-radius: var(--radius-sm);">
              ${protectedSvg}
            </div>
      ...
      <div class="step-pane animate-fade-in">
        ...
        <div class="card" style="display: flex; align-items: center; justify-content: space-between; flex-wrap: wrap; gap: 20px; background-color: var(--brand-surface-subtle); margin-bottom: 28px;">
          ...
          <span class="badge badge-active" style="padding: 8px 16px; font-size: 0.9rem;">
            ✅ ${isAr ? 'اجتاز فحص الأمان بنجاح' : 'Passed Privacy Audit'}
          </span>
        </div>
  ...
  renderStep6() {
    ...
    return `
      <div class="step-pane animate-fade-in" style="display: flex; flex-direction: column; align-items: center;">
        <div style="font-size: 2.8rem; margin-bottom: 12px; color: var(--brand-accent);">🎉</div>
        ...
        <div class="proof-mobile-mockup" style="margin-bottom: 28px;">
          <div class="proof-brand-header">
            ...
            <span style="font-size: 0.8rem; font-weight: 700; color: var(--brand-accent);">🟢 ${I18N.t('statusActive')}</span>
          </div>
          ...
          <div style="font-size: 0.78rem; color: var(--text-muted); line-height: 1.4; border-top: 1px solid var(--border-light); padding-top: 12px; width: 100%;">
            <div><strong>${isAr ? 'الجهة المستلمة:' : 'Recipient:'}</strong> ${isAr ? proof.recipientNameAr : proof.recipientNameEn}</div>
